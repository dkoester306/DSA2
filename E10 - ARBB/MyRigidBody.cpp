...
void MyRigidBody::SetModelMatrix(matrix4 a_m4ModelMatrix)
{
	...
	//your code goes here---------------------
	vector3 boxVerticies[8];

	// the verticies for the top and back faces of the box
	boxVerticies[0] = m_v3MinL;
	boxVerticies[1] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MinL.z);
	boxVerticies[2] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MinL.z);
	boxVerticies[3] = vector3(m_v3MaxL.x, m_v3MaxL.y, m_v3MinL.z);

	boxVerticies[4] = vector3(m_v3MinL.x, m_v3MinL.y, m_v3MaxL.z);
	boxVerticies[5] = vector3(m_v3MaxL.x, m_v3MinL.y, m_v3MaxL.z);
	boxVerticies[6] = vector3(m_v3MinL.x, m_v3MaxL.y, m_v3MaxL.z);
	boxVerticies[7] = m_v3MaxL;

	// oritent the local points correctly in the world
	for (uint i = 0; i < 8; ++i)
	{
		boxVerticies[i] = vector3(m_m4ToWorld * vector4(boxVerticies[i], 1.0f));
	}


	
	m_v3MaxG = m_v3MinG = boxVerticies[0];

	// find the new min and max for the global coordinates
	for (uint i = 1; i < 8; ++i)
	{
		if (m_v3MaxG.x < boxVerticies[i].x) 
			m_v3MaxG.x = boxVerticies[i].x;
		else if (m_v3MinG.x > boxVerticies[i].x)
			m_v3MinG.x = boxVerticies[i].x;

		if (m_v3MaxG.y < boxVerticies[i].y) 
			m_v3MaxG.y = boxVerticies[i].y;
		else if (m_v3MinG.y > boxVerticies[i].y) 
			m_v3MinG.y = boxVerticies[i].y;

		if (m_v3MaxG.z < boxVerticies[i].z) 
			m_v3MaxG.z = boxVerticies[i].z;
		else if (m_v3MinG.z > boxVerticies[i].z) 
			m_v3MinG.z = boxVerticies[i].z;
	}
